...
class TestGraphAbstractMethods:
    ...
    def test_add_node_raises_not_implemented(self) -> None:
        ...
        with pytest.raises(NotImplementedError, match="Subclasses must implement add_node"):
            graph.add_node("test")
    ...
    def test_remove_node_raises_not_implemented(self) -> None:
        ...
        with pytest.raises(NotImplementedError, match="Subclasses must implement remove_node"):
    ...
    def test_iter_edges_raises_not_implemented(self) -> None:
        ...
        with pytest.raises(NotImplementedError, match="Subclasses must implement _iter_edges"):
...
class TestGraphExportMermaid:
    ...
    def test_to_mermaid_with_node_label_fn(
        self, empty_graph: ConcreteGraph
    ) -> None:
    ...
    def test_to_mermaid_with_edge_label_fn(
        ...
        empty_graph.add_edge("A", "B", metadata=MockEdgeMetadata(weight=5, label="test"))
        
        mermaid = empty_graph.to_mermaid(
            edge_label_fn=lambda m: m.label if m else None
        )
        
        assert "test" in mermaid

    def test_to_mermaid_truncates_long_labels(
        self, empty_graph: ConcreteGraph
    ) -> None:
    ...
    def test_to_mermaid_special_chars_in_labels(
        ...
        empty_graph.add_node("test", value=MockNodeValue(name='Test "quoted" (parens) {braces}'))
        ...
        assert '"' not in mermaid.split('["')[1].split('"]')[0] if '["' in mermaid else True

    def test_to_mermaid_plus_sign_replaced(
        self, empty_graph: ConcreteGraph
    ) -> None:
...
class TestGraphExportJson:
    ...
    def test_to_json_dict_with_model_dump(
        self, empty_graph: ConcreteGraph
    ) -> None:
    ...
    def test_to_json_dict_with_str_value(
        self, empty_graph: ConcreteGraph
    ) -> None:
    ...
    def test_to_json_dict_edge_with_model_dump(
        ...
        empty_graph.add_edge(
            "A", "B", 
            metadata=MockEdgeMetadata(weight=10, label="test")
        )
    ...
    def test_to_json_dict_vars_exception(
        self, empty_graph: ConcreteGraph
    ) -> None:
...
class TestGraphExportAsciiTree:
    ...
    def test_to_ascii_tree_with_node_label_fn(
        self, empty_graph: ConcreteGraph
    ) -> None:
        """node_label_fn should customize node labels."""
    ...
    def test_to_ascii_tree_with_edge_label_fn(
        ...
        """edge_label_fn should customize edge labels."""
        empty_graph.add_edge("A", "B", metadata=MockEdgeMetadata(weight=5, label="edge_label"))
    ...
    def test_to_ascii_tree_node_label_exception(
        ...
        tree = empty_graph.to_ascii_tree(
            ...
            node_label_fn=bad_fn
        )
        
        # Should fall back to node_id
        assert "test_node" in tree

    def test_to_ascii_tree_nonexistent_root(
        self, empty_graph: ConcreteGraph
    ) -> None:
...
class TestGraphJsonEdgeCases:
    ...
    def test_to_json_dict_edge_vars_exception(
        self, empty_graph: ConcreteGraph
    ) -> None:
