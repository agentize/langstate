...
def sample_schema_path() -> Path:
    ...
    return Path(__file__).parent.parent.parent.parent / "data" / "schemas" / "registeration.yaml"
...
class TestOpenAPIReaderRead:
    ...
    def test_read_unknown_extension_yaml_content(self, temp_unknown_ext_file: Path) -> None:
    ...
    def test_read_unknown_extension_json_fallback(self, temp_unknown_ext_json_file: Path) -> None:
    ...
    def test_read_file_not_found_raises(self) -> None:
        ...
        with pytest.raises(FileNotFoundError, match="OpenAPI specification file not found"):
...
class TestOpenAPIReaderParseSchema:
    ...
    def test_parse_schema_with_x_sup_metadata(self) -> None:
        ...
        assert result["event"].metadata["x-sup"]["constraints"][0]["requires"] == ["user_id"]
...
class TestOpenAPIReaderIntegration:
    ...
    def test_read_full_registration_schema(
        self,
        sample_schema_path: Path
    ) -> None:
...
"""
    ...
    def test_parse_schema_property_with_non_standard_ref(self) -> None:
        ...
        schema = {
            "type": "object",
            "properties": {
                "field": {"$ref": "#/definitions/CustomType"}
            }
        }
...
"""
        ...
        assert schema.root["status"].validation_rules["enum"] == ["active", "inactive", "pending"]
