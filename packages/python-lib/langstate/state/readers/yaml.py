...
def _mk_self_constraints(
    field_id: str, prop_schema: Dict[str, Any]
) -> List[Constraint]:
            ...
            Constraint(
                conditions=[
                    ValueTypeCondition(
                        allowed=vt,
                        info=Info(
                            name="type", description=f"Type constraint for {field_id}"
                        ),
                    )
                ]
            )
    ...
    if (
        "enum" in prop_schema
        and isinstance(prop_schema["enum"], list)
        and prop_schema["enum"]
    ):
        out.append(
            Constraint(
                conditions=[
                    AllowDisallowCondition(
                        ...
                        info=Info(
                            name="enumeration",
                            description=f"Enumeration constraint for {field_id}",
                        ),
                    )
                ]
            )
            ...
            Constraint(
                conditions=[
                    RegexCondition(
                        ...
                        info=Info(
                            name="regex", description=f"Regex constraint for {field_id}"
                        ),
                    )
                ]
            )
            ...
            Constraint(
                conditions=[
                    RangeCondition(
                        ...
                        info=Info(
                            name="range", description=f"Range constraint for {field_id}"
                        ),
                    )
                ]
            )
...
def _get_properties_recursively(
    ...
    for prop_name, prop_schema_raw in properties.items():
        ...
        prop_schema = (
            _schema_to_dict(prop_schema_raw)
            if not isinstance(prop_schema_raw, dict)
            else prop_schema_raw
        )
        ...
        if prop_schema.get("type") == "object" and "properties" in prop_schema:
            ...
            nested_props = _get_properties_recursively(
                nested_entity, prop_schema, api, prefix=field_id, visited=visited
            )
            result.update(nested_props)
        ...
        if prop_schema.get("type") == "array":
            ...
            if (
                items
                and (items.get("type") == "object" if isinstance(items, dict) else True)
                or items_has_props
            ):
                ...
                nested_props = _get_properties_recursively(
                    nested_entity, items, api, prefix=field_id, visited=visited
                )
                result.update(nested_props)
...
def load_schema_from_openapi_yaml(
    doc: str | Path, root_entity: Optional[str] = None
) -> Schema:
    """
    ...
    if not root_entity:
        raise ValueError(
            "root_entity not specified. Either provide it as a parameter or include x-sup.root_entity in the YAML"
        )
    ...
    if root_entity not in api.components.schemas:
        ...
        raise ValueError(
            f"Root entity '{root_entity}' not found in components.schemas. Available: {available}"
        )
    ...
    for field_id, (prop_schema, entity_name) in all_properties.items():
        info = Info(name=field_id, description=prop_schema.get("description", ""))
    ...
    for field_id, property_obj in properties.items():
        property_nodes.append(
            DirectedAcyclicHypergraphNode[Field, Constraint](
                id=field_id, value=property_obj  # Use Property.id as node ID (not UUID)
            )
        )
    ...
    def _get_xsup_from_raw(
        field_id: str, raw_doc: Dict[str, Any]
        ...
        parts = field_id.split(".")
        if not parts or parts[0] not in raw_doc.get("components", {}).get(
            "schemas", {}
        ):
            return None
            ...
            # Look in items (for arrays)
                ...
                if isinstance(items, dict) and "allOf" in items:
                    ...
                    for allof_item in items["allOf"]:
                        ...
                        if (
                            "properties" in allof_item
                            and part in allof_item["properties"]
                        ):
    ...
    def _extract_constraints_for_property(
        field_id: str, raw_doc: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
    ...
    def _normalize_constraint_item(
        dep_field_id: str, item: Dict[str, Any]
    ) -> Optional[Tuple[List[str], str, Constraint]]:
        ...
        if "property_type" in payload:
            ...
            if isinstance(pt, dict):
                conditions.append(
                    ValueTypeCondition(
                        info=Info(
                            name="property_type", description="Property type constraint"
                        ),
                        **pt,
                    )
                )
        ...
        if "status" in payload:
            ...
            if isinstance(st, dict):
                conditions.append(
                    StatusCondition(
                        info=Info(name="status", description="Status constraint"), **st
                    )
                )
            ...
            if isinstance(en, list):
                conditions.append(
                    AllowDisallowCondition(
                        allowed=en,
                        info=Info(
                            name="enumeration", description="Enumeration constraint"
                        ),
                    )
                )
                    ...
                    conditions.append(
                        ValueSimilarityCondition(
                            ...
                            info=Info(
                                ...
                                description="Value similarity constraint",
                            ),
                        )
                    )
                else:
    ...
    for c in root_constraints:
        ...
        normalized = _normalize_constraint_item(
            (
                dep_rel
                if dep_rel.startswith(f"{root_entity}.")
                else f"{root_entity}.{dep_rel}"
            ),
            c,
        )
        if not normalized:
        ...
        if dep_id in properties and all(s in properties for s in source_ids):
            schema.add_hyperedge(
                sources=source_ids, target_id=dep_id, metadata=meta, check_cycle=True
            )
